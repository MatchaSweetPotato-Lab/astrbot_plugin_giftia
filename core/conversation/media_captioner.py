...
class MediaCaptioner:
    ...
    async def inspect_media(
        ...
        """Inspect or refresh an image, audio, or video caption.

        Args:
            hash_val: Media identifier.
            question: Optional focus for the caption.
            start_time: Video clip start in seconds.
            bot_name: Bot requesting the caption.
            group_or_user_id: Conversation identifier.
            duration: Optional video clip length in seconds.

        Returns:
            The media record and a formatted caption or error.
        """
        media_caption = await self.plugin.data_cache.get_caption_by_hash(hash_val)
        if not media_caption:
            logger.warning(
                f"[Giftia] inspect_media 失败：未找到对应的媒体缓存 hash={hash_val}"
            )
            return None, f"未找到 media_id 为 [{hash_val}] 的媒体数据记录"

        media_type = str(getattr(media_caption, "media_type", "") or "image").lower()
        logger.info(
            f"[Giftia] inspect_media: bot_name={bot_name}, hash={hash_val}, type={media_type}, question={question}, start_time={start_time}, duration={duration}"
        )

        # Explicit video durations must not reuse captions from a different clip.
        if (
            not question
            and start_time == 0
            and (media_type != "video" or duration is None)
            ...
            if media_type == "video":
                ...
                return media_caption, "；".join(
                    parts
                ) if parts else media_caption.caption
            else:  # image
                if media_caption.caption:
        ...
        try:
            if media_type == "video":
                if not getattr(
                    self.plugin.call_llm, "video_caption_provider_ids", None
                ):
                    return (
                        media_caption,
                        "未配置视频转述模型 (video_caption_provider_ids)，无法查看视频。",
                    )
            ...
            if media_type == "audio":
                if not getattr(
                    self.plugin.call_llm, "audio_caption_provider_ids", None
                ):
                    return (
                        media_caption,
                        "未配置语音转述模型 (audio_caption_provider_ids)，无法查看语音。",
                    )
                audio_urls = (
                    [str(cache_file)] if cache_file.exists() else [media_caption.url]
                )
                if audio_urls and audio_urls[0]:
                    ...
                    if transcribed:
                        ...
                        return media_caption, "；".join(
                            parts
                        ) if parts else "语音转述完成但无有效文本"
                return media_caption, "语音文件缺失或转述失败"

            else:  # image media
                if not getattr(
                    self.plugin.call_llm, "image_caption_provider_ids", None
                ):
                    return (
                        media_caption,
                        "未配置图片转述模型 (image_caption_provider_ids)，无法查看图片。",
                    )
                image_bytes = None
    ...
    async def transcribe_video_media(
        ...
        """Cache, clip, and transcribe a video on demand.

        Args:
            media_caption: Video metadata and any existing caption.
            start_time: Video clip start in seconds.
            question: Optional focus for the caption.
            bot_name: Bot requesting the caption.
            group_or_user_id: Conversation identifier.
            duration: Optional clip length, capped by the configured threshold.
        ...
        if not local_video_path.exists():
            if url.startswith("http://") or url.startswith("https://"):
                logger.info(
                    f"[Giftia] 下载视频文件进行分析: hash={media_caption.hash_val}"
                )
        ...
        if video_duration > clip_duration or start_time > 0 or duration is not None:
            clip_output = (
                cache_dir
                / f"{media_caption.hash_val}_clip_{start_time}_{clip_duration}.mp4"
            )
        ...
        if os.path.exists(target_video_file):
            ...
            if actual_size > VIDEO_AUTO_COMPRESS_THRESHOLD_BYTES:
                compressed_output = (
                    cache_dir / f"{Path(target_video_file).stem}_compressed.mp4"
                )
                logger.info(
        ...
        if (
            not os.path.exists(target_video_file)
            or os.path.getsize(target_video_file) == 0
        ):
